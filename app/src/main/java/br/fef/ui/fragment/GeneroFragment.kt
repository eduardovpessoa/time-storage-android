...
class GeneroFragment : Fragment() {
    ...
    private var generoList: List<Genero> = ArrayList()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val request: Call<List<Genero>> = api.getAllGeneros()
        request.enqueue(object : Callback<List<Genero>> {
            ...
            override fun onResponse(call: Call<List<Genero>>?, response: Response<List<Genero>>?) {
                if (response?.code() == 200) {
                    ...
                    recyclerGeneric.adapter = GeneroAdapter(generoList)
                }
            }
        })
